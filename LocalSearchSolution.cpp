...
void NearestNeighbor::heuristic_nearest_neighbor()
{
    ...
    for (int id_jour = 0; id_jour < this->instance->get_Nombre_Jour(); ++id_jour){
        ...
        cout << "@ - @ Day Max Distance = " << this->instance->get_POI_Duree_Max_Voyage(id_jour) << endl;

        //cout << "# - # Intermadiate = " << all_hotel.size() << " - " << id_jour + 1 << endl;
...
void NearestNeighbor::nearest_neighbor()
{
    ...
    for (int id_jour = 0; id_jour < this->instance->get_Nombre_Jour(); ++id_jour){
        ...
        bool hotel = true;
        this->total_distance_for_trip = 0.0f;
        float position_best_poi_not_found = -1;
        ...
        if (id_jour < this->hotel_Intermedaire.size()){
            ...
            while(iteration < maxIteration){
                ...
                if (hotel){
                    ...
                    if (index_best_poi != -1){
                        ...
                        cout << "# - # Best Poi not found for intermediate hotel !" << endl;
                    }
                    ...
                    if (index_best_poi != -1){
                            ...
                            if (distance_between_poi < distance_max_jour){
                                if(distance_between_poi < this->instance->get_POI_Heure_fermeture(index_best_poi)){
                                    ...
                                    if (distance_between_poi + distance_poi_last_poi + last_distance_poi_hotel < distance_max_jour){
                                        //cout << "# - # Ouverture/Fermeture POI : " << this->instance->get_POI_Heure_ouverture(index_best_poi) << " - " << this->instance->get_POI_Heure_fermeture(index_best_poi) << " ( " <<  index_best_poi << " )" << " - Distance : " << distance_between_poi << endl;
                                        if (!this->poi_already_visited(index_best_poi)){
                                            //cout << "Which POI : " << index_best_poi << endl;
                                            sequence_jour.push_back(index_best_poi);
                        ...
                        int poi = find_nearest_poi_not_visited(current_position, total_distance_for_trip, id_jour);

                        //cout << "@ - @ Nearest POI : " << poi << " - Distance : " << this->instance->get_distance_POI_POI(current_position, poi) << endl;
                        //current_position = poi;
                        //total_distance_for_trip += this->instance->get_distance_POI_POI(current_position, poi);
            ...
            current_position = hotel_Intermedaire[id_jour];
            hotel = true;
            this->total_distance_for_trip = 0.0f;
            sequence_jour.clear();
            ...
            while(movable){
                ...
                if (hotel){
                    ...
                    if (index_best_poi != -1){
                        ...
                        cout << "# - # Best Poi not found for last day !" << endl;
                    }
                }else{
                    //cout << "# - # About POI" << endl;
                    ...
                    if (index_best_poi != -1){
                        ...
                        cout << "# - # Not Found POI for last day" << endl;
                    }
...
int NearestNeighbor::find_nearest_poi_not_visited(int poi_index, float total_distance, int id_jour)
{
    int nearest_poi = -1;
    float nerest_distance = -1.0;
    ...
    for (int p = 0; p < this->instance->get_Nombre_POI(); ++p){
        if (!poi_already_visited(p) && p != poi_index){
            float distance_poi_poi = total_distance + this->instance->get_distance_POI_POI(poi_index,p);
            if (distance_poi_poi < this->instance->get_POI_Heure_fermeture(p)){
                cout << "@ - @ POI = " << p << " - Distance Max = " << distance_max_by_day << " - Distance = " << distance_poi_poi << endl;
                if (distance_poi_poi < distance_max_by_day){
                    if (nerest_distance < distance_poi_poi){
                        nerest_distance = distance_poi_poi;
                        cout << "? @ - @ POI = " << p << endl;
                        nearest_poi = p;
                    } 
                }
